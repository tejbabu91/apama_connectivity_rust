/*
 * $Copyright (c) 2019-2020 Software AG, Darmstadt, Germany and/or Software AG USA Inc., Reston, VA, USA, and/or its subsidiaries and/or its
 * affiliates and/or their licensors.$ Use, reproduction, transfer, publication or disclosure is prohibited except as specifically provided
 * for in your License Agreement with Software AG $Revision: 321386 $ $Date: 2017-12-14 10:26:04 +0000 (Thu, 14 Dec 2017) $
 */
...
sag_underlying_data_t* create_cpp_data_t_list_t(sag_underlying_vector_t* val) {
	...
	auto ret = reinterpret_cast<sag_underlying_data_t*>(new data_t(std::move(*l_class)));
	delete l_class;
	return ret;
}
...
sag_underlying_data_t* create_cpp_data_t_map_t(sag_underlying_map_t* val) {
	...
	auto ret = reinterpret_cast<sag_underlying_data_t*>(new data_t(std::move(*m_class)));
	delete m_class;
	return ret;
}
...
void free_cpp_message_t(sag_underlying_message_t* m) {
	Message* c_m = reinterpret_cast<Message*>(m);
	delete c_m;
}
