...
impl Codec for DiagCodec {
    ...
    fn deliverMessageTowardsTransport(&mut self, msg: Message) {
        println!("DiagnosticCodec[{}] Towards Transport: {:?} / {}", self.tag, msg.metadata, msg.payload);
        self.transport_side.sendMessageTowardsTransport(msg);
    }
    fn deliverMessageTowardsHost(&mut self, msg: Message) {
        println!("DiagnosticCodec[{}] Towards Host: {:?} / {}", self.tag, msg.metadata, msg.payload);
    ...
    fn new(host_side: HostSide, transport_side: TransportSide, params: CodecConstructorParameters) -> Box<dyn Codec> {
        let tag: String;
        ...
        let result = Box::new(DiagCodec{host_side, transport_side, params, tag});
        println!("DiagnosticCodec[{}] Created: {:?}", result.tag, result.params.getConfig());
        result
    }
}
